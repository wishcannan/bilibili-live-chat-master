<template>
  <div class="danmaku-item" :class="{ hidden: isHidden }">
    ...
    <div v-if="type === 'message'" class="danmaku-content">
      ...
      <span class="danmaku-author-name with-colon" :class="[{ anchor: isAnchor, owner: isOwner},'guard'+(privilege_type===0?'0':privilege_type+'')]">{{ uname }}</span>
      <span class="danmaku-reply-name" v-if="reply_uname">@{{reply_uname}}</span>
